...
public class Runner {
    ...
    public static int CreatureBoxes = 10;
    ...
    public static void main(String[] args)
    {
        ...
        playingBoard[0][0].enterRoom(player1, version);
        Scanner in = new Scanner(System.in);
        while(gameOn)
        {
            System.out.println("Where would you like to move? (Choose N, S, E, W)");
            String move = in.nextLine();
            if(validMove(move, player1, playingBoard, version))
            {
    ...
    public static boolean validMove(String move, Person p, Board[][] map, Typing version)
    {
        ...
        switch (move) {
            case "n":
                if (p.getxLoc() > 0)
                {
                    ...
                    map[p.getxLoc()-1][p.getyLoc()].enterRoom(p, version);
                    return true;
                }
                else
                {
            ...
            case "e":
                ...
                {
                    map[p.getxLoc()][p.getyLoc()].leaveRoom(p);
                    map[p.getxLoc()][p.getyLoc() + 1].enterRoom(p, version);
                    return true;
                }
                else
                {
